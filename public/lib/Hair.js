...
HairStrand.prototype.render = function(matrixWorld)
{
  ...
  for(let i = 0; i < this.control_vertices.length/3; i++){
    this.control_vertices[3*i] += Math.random() * 0.006 - 0.003;
    this.control_vertices[3*i + 2] += Math.random() * 0.006 - 0.003;
  }
  this.generateBezierControlVertices();
  this.final_vertices = this.generateFinalVertices(8); //8 is the number of verts between each pair of control points
  this.draw();
...
HairStrand.prototype.generateBezierControlVertices = function()
{
    this.bezier_control_vertices = [];
    ...
    if(this.bezier_control_vertices.length != ((3 * (this.control_vertices.length - 6)) + 12)){
      ...
      console.log("should be: " + ((3 * (this.control_vertices.length - 6)) + 12));
    }
};

HairStrand.prototype.generateFinalVertices = function(num_points)
{
  let output = [];
  for(let i = 0; i < this.bezier_control_vertices.length - 1; i+=3){
    ...
    let p2 = new Vector3([this.bezier_control_vertices[3*(i+1)], this.bezier_control_vertices[3*(i+1) + 1], this.bezier_control_vertices[3*(i+1) + 2]]);
    let p3 = new Vector3([this.bezier_control_vertices[3*(i+2)], this.bezier_control_vertices[3*(i+2) + 1], this.bezier_control_vertices[3*(i+2) + 2]]);
    let p4 = new Vector3([this.bezier_control_vertices[3*(i+3)], this.bezier_control_vertices[3*(i+3) + 1], this.bezier_control_vertices[3*(i+3) + 2]]);
    this.generateBezierPoints(p1, p2, p3, p4, num_points, output);
  }
  return new Float32Array(output);
};

HairStrand.prototype.generateBezierPoints = function(p1, p2, p3, p4, num_points, array)
{
  for(let i = 0; i < num_points; i++){
    let t = i / (num_points - 1);
    let x = (Math.pow(1-t, 3) * p1.elements[0])
          + (3 * Math.pow(1-t, 2) * t * p2.elements[0])
          + (3 * Math.pow(t, 2) * (1 - t) * p3.elements[0])
          + (Math.pow(t, 3) * p4.elements[0]);

    let y = (Math.pow(1-t, 3) * p1.elements[1])
          + (3 * Math.pow(1-t, 2) * t * p2.elements[1])
          + (3 * Math.pow(t, 2) * (1 - t) * p3.elements[1])
          + (Math.pow(t, 3) * p4.elements[1]);

    let z = (Math.pow(1-t, 3) * p1.elements[2])
